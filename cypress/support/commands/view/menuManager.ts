...
Cypress.Commands.add('pageIntegrations', () => {
  cy.menuItem(menuItem.manager.config)
    ...
    .click({ force: true });
  cy.href(Cypress.env('ROUTERS').integration)
    .should('have.text', menuItem.manager.integration)
    .click({ force: true });
...
Cypress.Commands.add('pageRegisterEnterprise', () => {
  cy.menuItem(menuItem.manager.manager)
    .realHover()
    .should('have.text', menuItem.manager.manager)
    .click({ force: true });
  cy.subitem(menuItem.manager.entity)
    .should('have.text', menuItem.manager.entity)
    .click({ force: true });
  cy.href(Cypress.env('ROUTERS').register)
    .should('have.text', menuItem.manager.registry)
    .realClick();
