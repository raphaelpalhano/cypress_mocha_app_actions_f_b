...
Cypress.Commands.add('modal', (message) => {
  return cy.get('[role="dialog"] div h2').should('have.text', message);
});
...
Cypress.Commands.add('menuItem', (value) => {
  return cy.get(`li:nth-child(${value}) span`);
});

Cypress.Commands.add('subitem', (value) => {
  return cy.get(`li:nth-child(${value})[data-testid="submenu-list-item"] button`);
});

Cypress.Commands.add('input', (value) => {
  return cy.get(`input[name*="${value}"]`);
});

Cypress.Commands.add('elementType', (value) => {
  return cy.get(`[type=${value}]`);
});
...
Cypress.Commands.add('role', (value) => {
